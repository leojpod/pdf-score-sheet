...
public class PDFScoreSheetServlet extends HttpServlet {
	...
	public PDF generatePDF(ScoreSheetData data) throws MalformedURLException,
			IOException {
		data.updateScore();
		...
		// team names
		x = 2.5f * 72;
		y = roasterPage.getHeight() - 72 * 3.34f
				+ titleStyle.getTextLineSpacing() * 2;
		...
		y = roasterPage.getHeight() - 3.53f * 72
				+ roasterStyle.getTextLineSpacing();
		float xHomeOffset = 1.37f * 72, xFstName = 0.44f * 72, xLstName = 1.62f * 72, xVisitorOffset = 4.76f * 72, ySkip = 0.22f * 72, yStart = y
				- ySkip;
		...
		periodStyle.setFont(
				new StandardFont(StandardFont.HELVETICABOLDOBLIQUE), 10);
		float timeOffset = 1.38f * 72, centerOffSet = 4.52f * 72, leftOffset = 1.86f * 72;
		...
		for (GameEvent evt : data.events) {
			...
			eventPage.setStyle(roasterStyle);
			eventPage.drawText(evt.time.minutes + ":" + evt.time.seconds,
					timeOffset, y);
	...
	public ScoreSheetData extractDataFromParam(String param) {
		// NOTE: for a real application you would want to have a better
		// parameter checking than here
		...
		/*
		 * JSON for team is something like: { name: "Smart bobcats", players:
		...
		for (JsonElement ply : homeTeamJson.get("players").getAsJsonArray()) {
			...
			// {firstName: "John", lastName: "smith", number:42}
			home.roaster.add(new Player(player.get("firstName").getAsString(),
					player.get("lastName").getAsString(), player.get("number")
							.getAsInt(), home));
		}
		visitor = new Team(visitorTeamJson.get("name").getAsString());
		for (JsonElement ply : visitorTeamJson.get("players").getAsJsonArray()) {
			JsonObject player = ply.getAsJsonObject();
			visitor.roaster.add(new Player(player.get("firstName")
					.getAsString(), player.get("lastName").getAsString(),
					player.get("number").getAsInt(), visitor));
		}
		...
		for (JsonElement evt : events) {
			...
			/*
			 * JSON should look like: { time:"00:15" type:"goal", info: { team:
			 * "home"/"visitor", scorer:42, with optionally: firstAssist: 11,
			 * secondAssist: 71} or { time:"55:58" type:"penalty", info: { team:
			 * "home"/"visitor", type:"high stick", minutes:"2+10", with
			 * optionally: number: 27 }
			 */
			int minutes, seconds;
			String[] timeBuffer = event.get("time").getAsString().split(":");
			minutes = Integer.parseInt(timeBuffer[0]);
			seconds = Integer.parseInt(timeBuffer[1]);
			...
			case goal:
				...
				try {
					if (team.equals("home")) {
						scorer = home.getPlayerByNumber(info.get("scorer")
								.getAsInt());
						if (info.has("firstAssist")) {
							try {
								firstAssist = home.getPlayerByNumber(info.get(
										"firstAssist").getAsInt());
								if (info.has("secondAssist")) {
									try {
										secondAssist = home
												.getPlayerByNumber(info.get(
														"secondAssist")
														.getAsInt());
						...
						scorer = visitor.getPlayerByNumber(info.get("scorer")
								.getAsInt());
						try {
							firstAssist = visitor.getPlayerByNumber(info.get(
									"firstAssist").getAsInt());
							if (info.has("secondAssist")) {
								try {
									secondAssist = visitor
											.getPlayerByNumber(info.get(
													"secondAssist").getAsInt());
								} catch (NumberFormatException e) {
									secondAssist = null;
								}
							}
					...
					scoreSheetData.events.add(new Goal(time, scorer,
							firstAssist, secondAssist));
				} catch (NumberFormatException e) {
					// malformed event (no goal scorer) let's ignore that
				}
				break;
			case penalty:
